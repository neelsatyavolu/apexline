...
assert.match(mainProcess, /DEFAULT_CODEX_MODEL = "gpt-6-sol"/, "Default Codex model should be GPT-6 Sol");
assert.doesNotMatch(mainProcess, /id: "gpt-5\.6-|id: "gpt-5\.5"/, "AI model list should not include GPT-5.5 or GPT-5.6");
...
assert.match(mainProcess, /reasoning:\s*\{\s*effort:\s*"low"\s*\}/, "Codex GPT-6 models should use low reasoning effort");
assert.match(mainProcess, /\^gpt-6-/, "Reasoning effort should apply to GPT-6 models");
assert.match(mainProcess, /\^codex:gpt-5\\\.\(\?:5\|6\)/, "Saved GPT-5.5 and GPT-5.6 preferences should migrate to the default Codex model");
...
assert.doesNotMatch(mainProcess, /id: "grok-4\.5"|DEFAULT_GROK_MODEL = "grok-4\.5"/, "AI model list should not keep Grok 4.5 as a selectable model");
assert.match(mainProcess, /model === "grok-4\.7"[\s\S]*body\.reasoning\s*=\s*\{\s*effort:\s*"high"\s*\}/, "Grok 4.7 should use high reasoning effort");
assert.match(mainProcess, /model === "grok-4\.6"[\s\S]*body\.reasoning\s*=\s*\{\s*effort:\s*"high"\s*\}/, "Grok 4.6 should use high reasoning effort");
assert.match(mainProcess, /if \(text === "grok:grok-4\.5"\) text = "grok:grok-4\.6";/, "Saved Grok 4.5 preference should migrate to Grok 4.6");
assert.match(settings, /Grok 4\.7/, "Settings should show Grok 4.7 as a model option");
assert.match(settings, /grok:grok-4\.7/, "Settings should use the grok-4.7 model id");
assert.match(settings, /Grok 4\.6/, "Settings should show Grok 4.6 as a model option");
assert.match(settings, /grok:grok-4\.6/, "Settings should use the grok-4.6 model id");
assert.match(settings, /value === "grok:grok-4\.5" \? "grok:grok-4\.6"/, "Settings should migrate a saved Grok 4.5 preference");
assert.doesNotMatch(settings, /label: "Grok 4\.5"/, "Settings should not offer Grok 4.5");
assert.doesNotMatch(settings, /Grok 4\.3|grok-4\.3|Local MLX|value: "local"/, "Settings should not offer Grok 4.3 or Local MLX");
...
assert.doesNotMatch(settings, /GPT-5\.5|GPT-5\.6|gpt-5\.5"|gpt-5\.6-/, "Settings should not offer GPT-5.5 or GPT-5.6");
...
assert.match(settings, /DEFAULT_AI_MODEL = "codex:gpt-6-sol"/, "Settings should default to GPT-6 Sol");
...
assert.match(settings, /\^codex:gpt-5\\\.\(\?:5\|6\)/, "Settings should migrate a saved GPT-5.5 or GPT-5.6 preference");
